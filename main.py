...
def get_start_info():
    ...
    while can_move_on == False:
        ...
        can_move_on = confirm_start_info(
            return_list
        )  # confirm_start_info() returns boolean
...
def confirm_start_info(returned_list):  # helper function of get_start_info()
    ...
    while True:
        confirmation_response = input(
            "Are you sure you want to translate Unit: "
            + unit
            + " from "
            + starting.capitalize()
            + "? "
        ...
        if confirmation_response in yeses:
            return (
                True  # get_start_info() will return this set of start language and unit
            )
...
def run_game(returned_list):
    ...
    while still_going:
        ...
        random_index = random.randint(
            0, (length_available_indexes_list - 1)
        )  # gets the random index in available_indexes_list
        prompt_index = available_indexes_list.pop(
            random_index
        )  # returns the value at that random index, and removes it from the list

        presented_choices = start_list[
            prompt_index
        ]  # the list of available words the user may translate from
        translated_choices = end_list[
            prompt_index
        ]  # the available translations for that word
...
def show_results(results):
    ...
    print(
        "You translated "
        + str(num_translated_correctly)
        + " correctly out of "
        + str(num_translated)
        + "."
    )
    accuracy = round(((num_translated_correctly / num_translated) * 100.0))
...
def play_game():
    ...
    start_info = (
        get_start_info()
    )  # start info will equal the return_list containing the starting language and unit
